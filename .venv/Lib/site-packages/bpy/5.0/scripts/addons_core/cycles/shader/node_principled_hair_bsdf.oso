...
# options: -q -O2 -ID:/.buildbpy/blender/intern/cycles/kernel/osl/shaders -ID:/.buildbpy/blender/lib/windows_x64/osl/share/OSL/shaders -o D:/.buildbpy/build_windows_Bpy_x64_vc16_Release/intern/cycles/kernel/osl/shaders/node_principled_hair_bsdf.oso
...
local	float	___370_x	%read{41,74} %write{40,65}
local	float	___370_roughness_fac	%read{59,84} %write{50,75}
local	color	___370_sigma	%read{60,85} %write{59,84}
...
local	float	___374_factor_random_color	%read{27,27} %write{26,26}
local	float	___374_melanin	%read{28,34} %write{27,31}
local	float	___374_eumelanin	%read{36,36} %write{33,33}
local	float	___374_pheomelanin	%read{37,37} %write{34,34}
local	color	___374_melanin_sigma	%read{61,61} %write{38,38}
local	color	___374_tint_sigma	%read{61,61} %write{60,60}
local	normal	___377_major_axis	%read{96,96} %write{92,95}
...
code Normal
# D:/.buildbpy/blender/intern/cycles/kernel/osl/shaders/node_principled_hair_bsdf.osl:32
#                                  normal Normal = Ng,
	assign		Normal Ng 	%filename{"D:/.buildbpy/blender/intern/cycles/kernel/osl/shaders/node_principled_hair_bsdf.osl"} %line{32} %argrw{"wr"}
code Offset
# D:/.buildbpy/blender/intern/cycles/kernel/osl/shaders/node_principled_hair_bsdf.osl:35
#                                  float Offset = radians(2),
	functioncall	$const3 3 	%filename{"D:/.buildbpy/blender/intern/cycles/kernel/osl/shaders/node_principled_hair_bsdf.osl"} %line{35} %argrw{"r"}
# D:/.buildbpy/blender/lib/windows_x64/osl/share/OSL/shaders/stdosl.h:65
# float  radians (float x)  { return x*(M_PI/180.0); }
	mul		Offset $const2 $const4 	%filename{"D:/.buildbpy/blender/lib/windows_x64/osl/share/OSL/shaders/stdosl.h"} %line{65} %argrw{"wrr"}
code ___main___
# D:/.buildbpy/blender/intern/cycles/kernel/osl/shaders/node_principled_hair_bsdf.osl:51
#   float random_value = 0.0;
	assign		random_value $const5 	%filename{"D:/.buildbpy/blender/intern/cycles/kernel/osl/shaders/node_principled_hair_bsdf.osl"} %line{51} %argrw{"wr"}
# D:/.buildbpy/blender/intern/cycles/kernel/osl/shaders/node_principled_hair_bsdf.osl:53
#   if (isconnected(Random)) {
	isconnected	$tmp1 Random 	%line{53} %argrw{"wr"}
	if		$tmp1 7 8 	%argrw{"r"}
# D:/.buildbpy/blender/intern/cycles/kernel/osl/shaders/node_principled_hair_bsdf.osl:54
#     random_value = Random;
	assign		random_value Random 	%line{54} %argrw{"wr"}
# D:/.buildbpy/blender/intern/cycles/kernel/osl/shaders/node_principled_hair_bsdf.osl:57
#     getattribute(AttrRandom, random_value);
	getattribute	$tmp2 AttrRandom random_value 	%line{57} %argrw{"wrw"}
# D:/.buildbpy/blender/intern/cycles/kernel/osl/shaders/node_principled_hair_bsdf.osl:61
...
# D:/.buildbpy/blender/intern/cycles/kernel/osl/shaders/node_principled_hair_bsdf.osl:62
...
# float  clamp (float x, float minval, float maxval) { return max(min(x,maxval),minval); }
	min		$tmp7 Coat $const6 	%filename{"D:/.buildbpy/blender/lib/windows_x64/osl/share/OSL/shaders/stdosl.h"} %line{141} %argrw{"wrr"}
	max		$tmp6 $tmp7 $const5 	%argrw{"wrr"}
# D:/.buildbpy/blender/intern/cycles/kernel/osl/shaders/node_principled_hair_bsdf.osl:62
#   float m0_roughness = 1.0 - clamp(Coat, 0.0, 1.0);
	sub		m0_roughness $const6 $tmp6 	%filename{"D:/.buildbpy/blender/intern/cycles/kernel/osl/shaders/node_principled_hair_bsdf.osl"} %line{62} %argrw{"wrr"}
# D:/.buildbpy/blender/intern/cycles/kernel/osl/shaders/node_principled_hair_bsdf.osl:63
#   float roughness = Roughness * factor_random_roughness;
	mul		roughness Roughness factor_random_roughness 	%line{63} %argrw{"wrr"}
# D:/.buildbpy/blender/intern/cycles/kernel/osl/shaders/node_principled_hair_bsdf.osl:64
...
# D:/.buildbpy/blender/intern/cycles/kernel/osl/shaders/node_principled_hair_bsdf.osl:69
#   if (parametrization == "Absorption coefficient") {
	eq		$tmp8 parametrization $const9 	%line{69} %argrw{"wrr"}
	if		$tmp8 21 90 	%argrw{"r"}
# D:/.buildbpy/blender/intern/cycles/kernel/osl/shaders/node_principled_hair_bsdf.osl:70
#     sigma = AbsorptionCoefficient;
	assign		sigma AbsorptionCoefficient 	%line{70} %argrw{"wr"}
# D:/.buildbpy/blender/intern/cycles/kernel/osl/shaders/node_principled_hair_bsdf.osl:72
#   else if (parametrization == "Melanin concentration") {
	eq		$tmp9 parametrization $const10 	%line{72} %argrw{"wrr"}
	if		$tmp9 62 90 	%argrw{"r"}
# D:/.buildbpy/blender/intern/cycles/kernel/osl/shaders/node_principled_hair_bsdf.osl:74
#     float factor_random_color = 1.0 + 2.0 * (random_value - 0.5) * RandomColor;
	...
	add		___374_factor_random_color $const6 $tmp12 	%argrw{"wrr"}
# D:/.buildbpy/blender/intern/cycles/kernel/osl/shaders/node_principled_hair_bsdf.osl:75
#     float melanin = Melanin * factor_random_color;
	mul		___374_melanin Melanin ___374_factor_random_color 	%line{75} %argrw{"wrr"}
# D:/.buildbpy/blender/intern/cycles/kernel/osl/shaders/node_principled_hair_bsdf.osl:78
#     melanin = -log(max(1.0 - melanin, 0.0001));
	sub		$tmp15 $const6 ___374_melanin 	%line{78} %argrw{"wrr"}
	max		$tmp14 $tmp15 $const11 	%argrw{"wrr"}
	log		$tmp13 $tmp14 	%argrw{"wr"}
	neg		___374_melanin $tmp13 	%argrw{"wr"}
# D:/.buildbpy/blender/intern/cycles/kernel/osl/shaders/node_principled_hair_bsdf.osl:81
#     float eumelanin = melanin * (1.0 - MelaninRedness);
	sub		$tmp16 $const6 MelaninRedness 	%line{81} %argrw{"wrr"}
	mul		___374_eumelanin ___374_melanin $tmp16 	%argrw{"wrr"}
# D:/.buildbpy/blender/intern/cycles/kernel/osl/shaders/node_principled_hair_bsdf.osl:82
#     float pheomelanin = melanin * MelaninRedness;
	mul		___374_pheomelanin ___374_melanin MelaninRedness 	%line{82} %argrw{"wrr"}
# D:/.buildbpy/blender/intern/cycles/kernel/osl/shaders/node_principled_hair_bsdf.osl:83
#     color melanin_sigma = sigma_from_concentration(eumelanin, pheomelanin);
	functioncall	$const12 39 	%line{83} %argrw{"r"}
# D:/.buildbpy/blender/intern/cycles/kernel/osl/shaders/node_principled_hair_bsdf.osl:14
#   return eumelanin * color(0.506, 0.841, 1.653) + pheomelanin * color(0.343, 0.733, 1.924);
	mul		$tmp18 ___374_eumelanin $const13 	%line{14} %argrw{"wrr"}
	mul		$tmp20 ___374_pheomelanin $const14 	%argrw{"wrr"}
	add		___374_melanin_sigma $tmp18 $tmp20 	%argrw{"wrr"}
# D:/.buildbpy/blender/intern/cycles/kernel/osl/shaders/node_principled_hair_bsdf.osl:86
#     color tint_sigma = sigma_from_reflectance(Tint, radial_roughness);
	functioncall	$const15 61 	%line{86} %argrw{"r"}
# D:/.buildbpy/blender/intern/cycles/kernel/osl/shaders/node_principled_hair_bsdf.osl:19
#   float x = azimuthal_roughness;
	assign		___370_x radial_roughness 	%line{19} %argrw{"wr"}
# D:/.buildbpy/blender/intern/cycles/kernel/osl/shaders/node_principled_hair_bsdf.osl:20
#   float roughness_fac = (((((0.245 * x) + 5.574) * x - 10.73) * x + 2.532) * x - 0.215) * x +
	mul		$tmp21 $const16 ___370_x 	%line{20} %argrw{"wrr"}
	add		$tmp22 $tmp21 $const17 	%argrw{"wrr"}
	mul		$tmp23 $tmp22 ___370_x 	%argrw{"wrr"}
	sub		$tmp24 $tmp23 $const18 	%argrw{"wrr"}
	mul		$tmp25 $tmp24 ___370_x 	%argrw{"wrr"}
	add		$tmp26 $tmp25 $const19 	%argrw{"wrr"}
	mul		$tmp27 $tmp26 ___370_x 	%argrw{"wrr"}
	sub		$tmp28 $tmp27 $const20 	%argrw{"wrr"}
	mul		$tmp29 $tmp28 ___370_x 	%argrw{"wrr"}
	add		___370_roughness_fac $tmp29 $const21 	%argrw{"wrr"}
# D:/.buildbpy/blender/intern/cycles/kernel/osl/shaders/node_principled_hair_bsdf.osl:22
#   color sigma = log3(c) / roughness_fac;
	functioncall	$const22 59 	%line{22} %argrw{"r"}
# D:/.buildbpy/blender/intern/cycles/kernel/osl/shaders/node_principled_hair_bsdf.osl:9
...
# D:/.buildbpy/blender/intern/cycles/kernel/osl/shaders/node_principled_hair_bsdf.osl:22
#   color sigma = log3(c) / roughness_fac;
	div		___370_sigma $tmp30 ___370_roughness_fac 	%line{22} %argrw{"wrr"}
# D:/.buildbpy/blender/intern/cycles/kernel/osl/shaders/node_principled_hair_bsdf.osl:23
#   return sigma * sigma;
	mul		___374_tint_sigma ___370_sigma ___370_sigma 	%line{23} %argrw{"wrr"}
# D:/.buildbpy/blender/intern/cycles/kernel/osl/shaders/node_principled_hair_bsdf.osl:87
#     sigma = melanin_sigma + tint_sigma;
	add		sigma ___374_melanin_sigma ___374_tint_sigma 	%line{87} %argrw{"wrr"}
# D:/.buildbpy/blender/intern/cycles/kernel/osl/shaders/node_principled_hair_bsdf.osl:89
#   else if (parametrization == "Direct coloring") {
	eq		$tmp37 parametrization $const25 	%line{89} %argrw{"wrr"}
	if		$tmp37 86 90 	%argrw{"r"}
# D:/.buildbpy/blender/intern/cycles/kernel/osl/shaders/node_principled_hair_bsdf.osl:90
...
# D:/.buildbpy/blender/intern/cycles/kernel/osl/shaders/node_principled_hair_bsdf.osl:20
#   float roughness_fac = (((((0.245 * x) + 5.574) * x - 10.73) * x + 2.532) * x - 0.215) * x +
	mul		$tmp38 $const16 ___370_x 	%line{20} %argrw{"wrr"}
	add		$tmp39 $tmp38 $const17 	%argrw{"wrr"}
	mul		$tmp40 $tmp39 ___370_x 	%argrw{"wrr"}
	sub		$tmp41 $tmp40 $const18 	%argrw{"wrr"}
	mul		$tmp42 $tmp41 ___370_x 	%argrw{"wrr"}
	add		$tmp43 $tmp42 $const19 	%argrw{"wrr"}
	mul		$tmp44 $tmp43 ___370_x 	%argrw{"wrr"}
	sub		$tmp45 $tmp44 $const20 	%argrw{"wrr"}
	mul		$tmp46 $tmp45 ___370_x 	%argrw{"wrr"}
	add		___370_roughness_fac $tmp46 $const21 	%argrw{"wrr"}
# D:/.buildbpy/blender/intern/cycles/kernel/osl/shaders/node_principled_hair_bsdf.osl:22
#   color sigma = log3(c) / roughness_fac;
...
# D:/.buildbpy/blender/intern/cycles/kernel/osl/shaders/node_principled_hair_bsdf.osl:22
#   color sigma = log3(c) / roughness_fac;
	div		___370_sigma $tmp47 ___370_roughness_fac 	%line{22} %argrw{"wrr"}
# D:/.buildbpy/blender/intern/cycles/kernel/osl/shaders/node_principled_hair_bsdf.osl:23
#   return sigma * sigma;
	mul		sigma ___370_sigma ___370_sigma 	%line{23} %argrw{"wrr"}
# D:/.buildbpy/blender/intern/cycles/kernel/osl/shaders/node_principled_hair_bsdf.osl:94
...
# D:/.buildbpy/blender/intern/cycles/kernel/osl/shaders/node_principled_hair_bsdf.osl:97
#   if (model == "Huang") {
	eq		$tmp58 model $const27 	%line{97} %argrw{"wrr"}
	if		$tmp58 97 98 	%argrw{"r"}
# D:/.buildbpy/blender/intern/cycles/kernel/osl/shaders/node_principled_hair_bsdf.osl:98
#     normal major_axis = Normal;
	assign		___377_major_axis Normal 	%line{98} %argrw{"wr"}
# D:/.buildbpy/blender/intern/cycles/kernel/osl/shaders/node_principled_hair_bsdf.osl:99
#     if (AspectRatio != 1.0) {
	neq		$tmp59 AspectRatio $const6 	%line{99} %argrw{"wrr"}
	if		$tmp59 96 96 	%argrw{"r"}
# D:/.buildbpy/blender/intern/cycles/kernel/osl/shaders/node_principled_hair_bsdf.osl:100
#       getattribute("geom:N", major_axis);
	getattribute	$tmp60 $const28 ___377_major_axis 	%line{100} %argrw{"wrw"}
# D:/.buildbpy/blender/intern/cycles/kernel/osl/shaders/node_principled_hair_bsdf.osl:102
#     BSDF = hair_huang(
	closure		BSDF $const29 ___377_major_axis sigma roughness Offset IOR AspectRatio Rlobe TTlobe TRTlobe 	%line{102} %argrw{"wrrrrrrrrrr"}
# D:/.buildbpy/blender/intern/cycles/kernel/osl/shaders/node_principled_hair_bsdf.osl:106
